...
async function generatePreview(content) {
  const prompt = `Please write a concise, engaging 1-paragraph preview for this blog post. The preview should be suitable for displaying on the home page and category listing pages. It should be longer and more detailed than a meta description, giving readers a better sense of what the post covers. Only return the preview text, nothing else.
...
function updatePreview(filepath, newPreview) {
  const content = fs.readFileSync(filepath, 'utf-8');
  const frontmatterMatch = content.match(/^(---\n)([\s\S]*?)(\n---\n)([\s\S]*)/);
  ...
  // Use YAML folded scalar (>) for multiline text to avoid quote escaping issues
  const yamlPreview = `>\n  ${newPreview}`;

  // Replace or add preview
  let updatedFrontmatter;
  if (frontmatter.match(/preview:/)) {
    // Replace existing preview (handle both quoted strings and folded scalars)
    updatedFrontmatter = frontmatter.replace(
      /preview:\s*(?:['"][^'"]*['"]|>[\s\S]*?)(?=\n\w+:|$)/,
      `preview: ${yamlPreview}`
    );
  } else {
    // Add preview after description
    updatedFrontmatter = frontmatter.replace(
      /(description:\s*['"][^'"]*['"])/,
      `$1\npreview: ${yamlPreview}`
    );
...
async function main() {
  ...
  if (slugArg) {
    ...
    const selection = await p.select({
      message: 'Select a post to generate a preview for:',
  ...
  const s = p.spinner();
  s.start('Generating preview with Claude...');

  let preview;
  try {
    preview = await generatePreview(postContent);
    s.stop('Preview generated!');
  } catch (error) {
    s.stop('Failed to generate preview');
  ...
  console.log('');
  p.log.message('Generated Preview:');
  console.log('');
  ...
  const wrappedLines = wrapText(preview);
  ...
  const confirm = await p.confirm({
    message: 'Update the post with this preview?',
    initialValue: true,
  });
  ...
  if (confirm) {
    try {
      updatePreview(filepath, preview);
      p.outro(`✅ Preview updated successfully!`);
    } catch (error) {
      p.cancel(`Failed to update file: ${error.message}`);
      process.exit(1);
    }
  } else {
    p.outro('Preview not saved.');
  }
}
